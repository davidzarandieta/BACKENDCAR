  ...
  if (err.errors.length > 0) {
    ...
    for (let i = 0; i < req.body.products.length; i++) {
      ...
      newOrder.dataValues.products.push({
        ...
        priceUnit: NewProd.price
      })
    }
    let precio = 0

    for (let i = 0; i < newOrder.dataValues.products.length; i++) {
      precio = precio + newOrder.dataValues.products[i].priceUnit * newOrder.dataValues.products[i].quantity
    }
    newOrder.price = precio
    if (newOrder.price > 10) {
      newOrder.shippingCosts = 0
    } else {
      const NewRest = await Restaurant.findByPk(req.body.restaurantId, {
        attributes: ['shippingCosts']
      })
      newOrder.shippingCosts = NewRest.shippingCosts
    }
    newOrder.price = newOrder.price + newOrder.shippingCosts
    try {
      const order = await newOrder.save()
      res.json(order)
      for (let i = 0; i < req.body.products.length; i++) {
        const prod = await Product.findByPk(req.body.products[i].productId, {
          attributes: ['price', 'id']
        })
        newOrder.addProduct(prod.id, { through: { quantity: req.body.products[i].quantity, unityPrice: prod.price } })
      }
    } catch (err) {
